"""
Unified Quantile Regression Tree implementation with multiple splitting criteria.
...
class QuantileRegressionTree:
    ...
    def _split_numeric(self, feature_idx, dataset, quantile):
        ...
        for v1, v2 in zip(values[:-1], values[1:]):
            ...
            if is_better(current_score, best_score):
                best_score, best_thr, best_groups = current_score, thr, {'<': left, '>=': right}
    ...
    def predict_interval(self, X, lower_quantile, upper_quantile):
        ...
        raise NotImplementedError("predict_interval requires access to training data. Use fit_interval method instead.")
