...
class QuantileRegressionTree:
    ...
    @staticmethod
    def _pinball_loss(y: np.ndarray, q: float, q_value: float) -> float:
        """
        Compute pinball (quantile) loss for predictions equal to `q_value`.
    ...
    def _evaluate_split_loss(
        ...
        """
        Evaluate a split by total pinball loss on both children.

        Uses `np.partition` to obtain the child quantile in O(n) expected time.
        ...
        def q_and_loss(y: np.ndarray) -> float:
            if y.size == 0:
                return 0.0
            k = int(np.floor(quantile * (y.size - 1)))
            part = np.partition(y, k)  # k-th element is in sorted position
            qv = part[k]
            return self._pinball_loss(y, quantile, qv)

        return q_and_loss(y_left) + q_and_loss(y_right)
    ...
    def _evaluate_split_r2(
        ...
        """
        Evaluate a split by R² using constant per-child quantile predictions.
        ...
        for y in (y_left, y_right):
            if y.size == 0:
                continue
            k = int(np.floor(quantile * (y.size - 1)))
            qv = np.partition(y, k)[k]
            preds.append(np.full(y.size, qv, dtype=float))
            labels.append(y)
    ...
    def _best_split_for_feature(
        ...
        for pos in np.nditer(cut_positions):
            ...
            if self.split_criterion == "loss":
                score = self._evaluate_split_loss(yL, yR, quantile)
            else:
                score = self._evaluate_split_r2(yL, yR, quantile)
    ...
    def _terminal_value(self, idx: np.ndarray, quantile: float) -> float:
        """
        Compute the leaf prediction as the sample quantile for indices `idx`.
        """
        y = self._y[idx]
        k = int(np.floor(quantile * (y.size - 1)))
    ...
    def _build_tree(self, indices: np.ndarray, quantile: float) -> None:
        ...
        while q:
            ...
            if depth >= self.max_depth or idx.size <= min_needed:
                node.prediction = self._terminal_value(idx, quantile)
            ...
            if (
                ...
                self.tree_nodes[node.node_id]["prediction"] = self._terminal_value(
                    idx, quantile
                )
    ...
    def _predict_one(self, row: pd.Series | np.ndarray) -> float:
        """
        Route a single sample down the stored split structure to get a value.
        """
        ...
        while True:
            ...
            if next_node is None:
                # Fallback: average available child predictions if any.
                preds = [c["prediction"]
                         for c in children if c["prediction"] is not None]
                if preds:
                    return float(np.mean(preds))
                return float(cur.get("prediction", np.nan))
