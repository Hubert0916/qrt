...
class QuantileTreeEnsemble:
    ...
    def predict(
        ...
        # Check if we're using QuantileRegressionModelTree (model-based)
        if self.tree_cls.__name__ == 'QuantileRegressionModelTree':
    ...
    def _predict_model_based(self, X_df: pd.DataFrame, quantile: float) -> np.ndarray:
        """
        Predict using model-based trees (e.g., QuantileRegressionModelTree).
